...
import SidePanel from './SidePanel'; // Or '../SidePanel' etc.
import EventCard from './EventCard';   // Or '../EventCard' etc.
...
function CalendarTab({ events = [], onEventClick, onEventUpdate }) { // Added onEventUpdate prop
  // --- State Declarations ---
  ...
  // State for Event Card in SidePanel
  ...
  const handleOpenEventPanel = (event) => {
    ...
    if (onEventClick) { // Call original onEventClick if provided
        onEventClick(event);
    }
  };
  ...
  const handleSaveEventInPanel = async (updatedEvent) => {
    if (onEventUpdate) {
      try {
        await onEventUpdate(updatedEvent); // Prop to handle actual update in parent
        setSelectedEventForPanel(updatedEvent); // Update event in panel
        // Optionally, provide feedback to the user
        // alert("Event saved successfully!");
        // The parent component should update the `events` prop, which will re-render CalendarTab
      } catch (error) {
        console.error("Failed to save event:", error);
        alert("Failed to save event. See console for details.");
      }
    } else {
      console.warn("CalendarTab: onEventUpdate prop not provided. Cannot save event changes.");
      alert("Save functionality is not fully configured (onEventUpdate missing).");
    }
    // Keep panel open to see changes or close if preferred:
  ...
  return (
    <div className="p-4 relative w-full zoom-90">
                    ...
                    >
                      ...
                      <div className={`mt-1 space-y-0.5 overflow-hidden text-ellipsis ${cellOpacityClass}`}>
                        ...
                        {eventsForDay.slice(0, cellItemsLimit - notesForDay.length).map((event) => {
                          ...
                          return (
                            <button // Changed from div to button to make it clearly clickable
                              key={`event-${event.id}`}
                              onClick={(e) => { // MODIFIED: Open Event Panel
                                e.stopPropagation();
                    ...
                    {infoModalContent.events.map((event) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {/* MODIFIED: Button to open Event Panel */}
                          <button
                              onClick={() => {
                                  // setInfoModalOpen(false); // Already handled by handleOpenEventPanel
                                  handleOpenEventPanel(event);
                              }}
      ...
      {noteModalOpen && createPortal(
        // ... Note Modal JSX remains the same ...
        <div
        ...
        >
            ...
            <div className="flex-grow overflow-y-auto pr-2 -mr-2 space-y-4">
                ...
                <input
                    id="noteTitle" type="text" value={newNote.title}
                ...
                <input
                    id="noteDate" type="date" value={newNote.date}
                ...
                <div className="flex space-x-2 flex-wrap gap-1">
                    ...
                    <button
                        key={color} type="button" style={{ backgroundColor: color }}
                ...
                <textarea
                    id="noteContent" value={newNote.content}
                    onChange={(e) => setNewNote({ ...newNote, content: e.target.value })}
                    className="w-full border border-gray-300 rounded-md p-2 shadow-sm focus:ring-blue-500 focus:border-blue-500"
                    placeholder="Add details (optional)" rows="4"
                />
                </div>
            </div>
            <div className="flex justify-end space-x-3 mt-6 border-t pt-4">
                <button
                type="button" onClick={closeNoteModal}
                className="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors"
                >
                Cancel
                </button>
                <button
                type="button" onClick={saveCalendarNote}
      ...
      {isEventPanelOpen && selectedEventForPanel && (
        ...
        >
          <EventCard
            ...
            setActiveEvent={handleCloseEventPanel} // Allows EventCard's close to close the panel
            // onDelete, onMoveLeft, onMoveRight could be passed here if needed
            // For simplicity, EventCard's internal actions might be sufficient or use hideActions
            hideActions={false} // Show EventCard's internal save/actions
          />
        </SidePanel>
      )}
      </div>
