...
function Sidebar({ currentTab, setCurrentTab, onExport }) {
  ...
  return (
    // Added `relative` and `z-30` to the Sidebar's main div.
    // `relative` is often needed for `z-index` to take effect properly on non-positioned elements.
    // `z-30` should be lower than the SidePanel's backdrop (typically z-40).
    <div className="w-64 bg-gray-900 text-white h-full flex flex-col shadow-lg relative z-30">
